...
interface CmdrTerminal extends Terminal {
    fit(): void;
}
...
export class Xterm {
    ...
    public async refreshToken() {
        try {
            ...
            console.error(`[cmdr] fetch ${this.options.tokenUrl}: `, e);
        }
    }
    ...
    public open(parent: HTMLElement) {
        ...
        window.term = terminal as CmdrTerminal;
        window.term.fit = () => {
            this.fitAddon.fit();
        };
        ...
        for (const [k, queryVal] of queryObj) {
            ...
            switch (typeof v) {
                ...
                case 'object':
                    ...
                    break;
                default:
                    console.warn(`[cmdr] maybe unknown option: ${k}=${queryVal}, treating as string`);
                    prefs[k] = queryVal;
                    break;
            }
        }
    ...
    private onSocketData(event: MessageEvent) {
        ...
        switch (cmd) {
            ...
            case Command.SET_PREFERENCES:
                ...
                break;
            default:
                console.warn(`[cmdr] unknown command: ${cmd}`);
                break;
        }
    }
            ...
            switch (key) {
                ...
                case 'disableLeaveAlert':
                    if (value) {
                        ...
                        console.log('[cmdr] Leave site alert disabled');
                    }
                    break;
                case 'disableResizeOverlay':
                    if (value) {
                        console.log('[cmdr] Resize overlay disabled');
                        this.resizeOverlay = false;
                    }
                    break;
                case 'disableReconnect':
                    if (value) {
                        console.log('[cmdr] Reconnect disabled');
                        this.reconnect = false;
                        this.doReconnect = false;
                    }
                    break;
                case 'enableZmodem':
                    if (value) console.log('[cmdr] Zmodem enabled');
                    break;
                case 'enableTrzsz':
                    if (value) console.log('[cmdr] trzsz enabled');
                    break;
                case 'trzszDragInitTimeout':
                    if (value) console.log(`[cmdr] trzsz drag init timeout: ${value}`);
                    break;
                case 'enableSixel':
                    if (value) {
                        terminal.loadAddon(register(new ImageAddon()));
                        console.log('[cmdr] Sixel enabled');
                    }
                    break;
                case 'closeOnDisconnect':
                    if (value) {
                        console.log('[cmdr] close on disconnect enabled (Reconnect disabled)');
                ...
                case 'titleFixed':
                    ...
                    console.log(`[cmdr] setting fixed title: ${value}`);
                    this.titleFixed = value;
                    document.title = value;
                    break;
                case 'isWindows':
                    if (value) console.log('[cmdr] is windows');
                    break;
                case 'unicodeVersion':
                    switch (value) {
                        case 6:
                        case '6':
                            console.log('[cmdr] setting Unicode version: 6');
                            break;
                        case 11:
                        case '11':
                        default:
                            console.log('[cmdr] setting Unicode version: 11');
                    ...
                    break;
                default:
                    console.log(`[cmdr] option: ${key}=${JSON.stringify(value)}`);
    ...
    private setRendererType(value: RendererType) {
        ...
        const enableCanvasRenderer = () => {
            ...
            try {
                ...
                console.log('[cmdr] canvas renderer loaded');
            } catch (e) {
                console.log('[cmdr] canvas renderer could not be loaded, falling back to dom renderer', e);
        ...
        const enableWebglRenderer = () => {
            ...
            try {
                ...
                console.log('[cmdr] WebGL renderer loaded');
            } catch (e) {
                console.log('[cmdr] WebGL renderer could not be loaded, falling back to canvas renderer', e);
                disposeWebglRenderer();
        ...
        switch (value) {
            ...
            case 'dom':
                disposeWebglRenderer();
                disposeCanvasRenderer();
                console.log('[cmdr] dom renderer loaded');
                break;
            default:
                break;
        }
    ...
    @bind
    public switchToSession(sessionId: string, preserveState: boolean = true): void {
        if (this.currentSessionId === sessionId) return;
        
        console.log(`[cmdr] Switching to session: ${sessionId}`);
        
        // Save current session state
        if (this.currentSessionId && preserveState && this.terminal) {
            this.saveSessionState(this.currentSessionId);
        }
        ...
        // Only clear terminal if we don't have saved state for this session
        // This preserves the current terminal content when switching to a new session
        const hasSavedState = this.sessionBuffers.has(sessionId);
        if (this.terminal && !hasSavedState) {
            // Only clear for completely new sessions
            this.terminal.clear();
        }
        
        // Restore session state if available
        if (preserveState && hasSavedState) {
            // Clear and restore saved content
            if (this.terminal) {
                this.terminal.clear();
                this.terminal.reset();
            }
    ...
    private saveSessionState(sessionId: string): void {
        ...
        try {
            ...
            this.sessionBuffers.set(sessionId, content);
            ...
            console.log(`[cmdr] Saved state for session: ${sessionId}`);
        } catch (error) {
            console.warn(`[cmdr] Failed to save session state for ${sessionId}:`, error);
        }
    }
    ...
    private restoreSessionState(sessionId: string): void {
        ...
        try {
            ...
            const savedBuffer = this.sessionBuffers.get(sessionId);
            if (savedBuffer) {
            ...
            if (savedState) {
                // Note: Some state restoration might need to be done after connection
                if (savedState.title && !this.titleFixed) {
                    document.title = savedState.title;
                    this.title = savedState.title;
                }
            }
            
            console.log(`[cmdr] Restored state for session: ${sessionId}`);
        } catch (error) {
            console.warn(`[cmdr] Failed to restore session state for ${sessionId}:`, error);
        }
    }
    ...
    public clearSessionState(sessionId: string): void {
        ...
        console.log(`[cmdr] Cleared state for session: ${sessionId}`);
    }
}
