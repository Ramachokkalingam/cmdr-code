...
static void print_help() {
  // clang-format off
  fprintf(stderr, "cmdr is a tool for sharing terminal over the web\n\n"
          "USAGE:\n"
          ...
          "Visit https://github.com/tsl0922/cmdr to get more information and report bugs.\n",
          CMDR_VERSION
  );
  // clang-format on
}
...
static struct server *server_new(int argc, char **argv, int start) {
  ...
  *ptr = '\0';  // null terminator
  ...
  // Initialize session manager for ChatGPT-style session management
  ts->session_mgr = session_manager_init();
  
  // Initialize persistent session registry
  ts->persistent_registry = session_registry_create(NULL);
  if (ts->persistent_registry) {
    session_registry_load_from_disk(ts->persistent_registry);
    lwsl_user("Persistent session registry initialized\n");
  } else {
    lwsl_err("Failed to initialize persistent session registry\n");
  }
...
static void server_free(struct server *ts) {
  ...
  if (ts->session_mgr != NULL) {
    session_manager_free(ts->session_mgr);
  }
  
  // Free persistent session registry
  if (ts->persistent_registry != NULL) {
    session_registry_save_all(ts->persistent_registry);
    session_registry_destroy(ts->persistent_registry);
    lwsl_user("Persistent session registry destroyed\n");
  }
...
static int parse_int(char *name, char *str) {
  ...
  if (errno != 0 || endptr == str) {
    fprintf(stderr, "cmdr: invalid value for %s: %s\n", name, str);
    exit(EXIT_FAILURE);
  }
  return (int)val;
}
...
// Timer callback for session maintenance
static void session_maintenance_timer_cb(lws_sorted_usec_list_t *sul) {
  // Perform session maintenance
  if (server && server->persistent_registry) {
    session_registry_maintenance(server->persistent_registry);
  }
  
  // Schedule next maintenance in 30 seconds
  lws_sul_schedule(context, 0, sul, session_maintenance_timer_cb, 30 * LWS_US_PER_SEC);
}

static lws_sorted_usec_list_t sul_maintenance;
  ...
  while ((c = getopt_long(start, argv, opt_string, options, NULL)) != -1) {
    switch (c) {
      ...
      case 'p':
        ...
        if (info.port < 0) {
          fprintf(stderr, "cmdr: invalid port: %s\n", optarg);
          return -1;
        }
        break;
      case 'i':
      ...
      case 'c':
        if (strchr(optarg, ':') == NULL) {
          fprintf(stderr, "cmdr: invalid credential, format: username:password\n");
          return -1;
        }
        char b64_text[256];
      ...
      case 's': {
        ...
        if (sig > 0) {
          ...
          fprintf(stderr, "cmdr: invalid signal: %s\n", optarg);
          return -1;
        }
      } break;
      case 'w':
...
#undef sc
      ...
      case 'P': {
        ...
        if (interval < 0) {
          fprintf(stderr, "cmdr: invalid ping interval: %s\n", optarg);
          return -1;
        }
      ...
      case 'f': {
        ...
        if (serv_buf_size < 0) {
          fprintf(stderr, "cmdr: invalid srv-buf-size: %s\n", optarg);
          return -1;
        }
      ...
      case 't':
        ...
        for (; optind < start && *argv[optind] != '-'; optind++) {
          ...
          if (key == NULL) {
            fprintf(stderr, "cmdr: invalid client option: %s, format: key=value\n", optarg);
            return -1;
          }
  ...
  if (server->command == NULL || strlen(server->command) == 0) {
    fprintf(stderr, "cmdr: missing start command\n");
    return -1;
  }
  ...
  char server_hdr[128] = "";
  sprintf(server_hdr, "cmdr/%s (libwebsockets/%s)", CMDR_VERSION, LWS_LIBRARY_VERSION);
  ...
  lwsl_notice("cmdr %s (libwebsockets %s)\n", CMDR_VERSION, LWS_LIBRARY_VERSION);
  print_config();
...
#define sig_count 2
  ...
  lws_sul_schedule(context, 0, &sul_maintenance, session_maintenance_timer_cb, 30 * LWS_US_PER_SEC);
  lwsl_notice("Session maintenance timer started\n");
